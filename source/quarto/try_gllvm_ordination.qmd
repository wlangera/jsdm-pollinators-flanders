...
$$
\begin{align}
\text{log}(\text{E}(y)) \sim \beta_0 + \beta_{mixed} + \beta_{wet} + \text{log}(X_{area}) + \text{log}(X_{effort}) \\

\Rightarrow \text{log} \left( \frac{\text{E}(y)}{X_{area} \times X_{effort}} \right) \sim \beta_0 + \beta_{mixed} + \beta_{wet}
\end{align}
$$
