...
We look at the ordination of the model without covariates.
...
site_scores <- getLV(model2, type = "residual")
species_loadings <- getLoadings(model2)

site_scores_final <- site_scores %>%
  bind_cols(plot_data_wide)
species_loadings_final <- species_loadings %>%
  as_tibble(rownames = "species")

ggplot(site_scores_final, aes(x = LV1, y = LV2)) +
  geom_point(aes(colour = habClass)) +
  geom_text(data = species_loadings_final, aes(label = species))
```
...
```{r}
ordiplot(ord_model1, symbols = TRUE, biplot = TRUE,
         s.colors = col_vect, s.cex = 0.5, pch = 16)
legend(
  "topright",
  legend = levels(plot_data_wide$habClass),
...
# Convergence
hist(my_model1$TMBfn$gr())

# Create ordination
ordiplot(my_model1, symbols = TRUE, biplot = TRUE,
         s.colors = col_vect, s.cex = 0.5, pch = 16)
legend(
  "topright",
  legend = levels(plot_data_wide$habClass),
...
plot_variance_partitioning(my_model1)


# Categorical predictor and offset
my_model2 <- gllvm(
  y = butterflies_data_wide[, -1],
  X = plot_data_wide,
  formula = ~(habClass | 1),
...
# Convergence
hist(my_model2$TMBfn$gr())

# Create ordination
ordiplot(my_model2, symbols = TRUE, biplot = TRUE,
         s.colors = col_vect, s.cex = 0.5, pch = 16)
legend(
  "topright",
  legend = levels(plot_data_wide$habClass),
...
# Variance partitioning
plot_variance_partitioning(my_model2)
...
if (file.exists(file.path(cache_path, "model5.Rds"))) {
  ...
  model5 <- gllvm(
    ...
    family = "ZINB",
    sd.errors = TRUE,
    rangeP = 100 # starting value range parameter
  )
...
There is something weird going on. The correlation goes down very fast.
...
tr_data <- trait_data_interim %>%
  ...
  # Sort species like in Y dataset
  arrange(factor(vernacularName, levels = names(y_data_traits[, -1]))) %>%
...
Model fit looks more or less good.
...
- In dry and mixed heath
  - Species that are smaller have more observations
  - Species that tolerate a warmer maximum temperature have more observations
- In dry
  - Species that tolerate a warmer minimum and mean temperature have more observations
