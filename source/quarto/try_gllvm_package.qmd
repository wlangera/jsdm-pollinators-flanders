...
We try a fourth-corner model with a reduced dataset without species for which we do not have trait data.

```{r}
# Select species
fourth_corner_specs <- intersect(names(butterflies_data_wide)[-1],
                                 trait_species)
y_data_traits <- butterflies_data_wide[, c("habID", fourth_corner_specs)]

# Remove sites with zeroes in y and X dataframes
...
```{r}
selected_traits <- c("temperatureMin", "temperatureMax", "temperatureMean",
                     "bodyLength")
```

We prepare trait data. We select the following traits and scale them: `r selected_traits`.
...
The previous assumes of species’ responses are fully due to traits. If the chosen traits are not important to determining species’ environmental responses, it will result in lack-of-fit. So, in this next model we relax that assumption by also including species-specific random effects.
